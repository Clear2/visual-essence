...
- 跳转后立即进入三栏对话工作台，不经过中间加载页；助手消息和右侧内容脉络会实时更新，分析完成后才出现可点击的“查看私教解读”
- 分析完成后可在底部继续提问；回答只使用已保存的视频转写和最近对话作为上下文；刷新或重新打开已完成会话时会直接恢复视频结果和消息，不会重新开始提取
- 提供固定底部输入框、桌面侧栏折叠，以及移动端会话与内容脉络抽屉
- 后端实时推送公开处理记录，前端只按执行顺序把后端确认的说明续写成一条连续叙事；运行态会在完成时原位替换，最终解读会加入基于完整转写的内容主线。界面不会根据事件标签或任意数据字段补写判断，也不展示阶段数量、耗时卡片或模型隐藏推理
- 连续处理记录结束只代表任务到达终态，不自动代表分析成功；只有同时取得转写和私教解读时才显示“视频分析完成”，仅返回元数据时显示“视频分析未完成”并提供未完成原因
...
- 视频分析未启用，或语音转写、LLM 执行失败时，会明确降级为仅返回元数据并说明原因；结果页不会提供空的私教解读入口，也不会拿视频标题冒充真实内容总结
